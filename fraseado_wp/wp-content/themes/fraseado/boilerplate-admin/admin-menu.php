...
/*	Define Boilerplate URI */
	#$h5bp_url = str_replace('http://', '//', get_template_directory_uri());
...
/*	2)	Add Admin Page CSS if on the Admin Page */
		...
		if ( ! function_exists( 'H5BP_admin_register_head' ) ):
			function H5BP_admin_register_head() {
				...
				echo "<script>jQuery(function() { jQuery('.wp-has-submenu li a:contains(\"Fundo\")').text('Rodapé'); } );</script>";
			}
...
/*	3)	Add "HTML5 Boilerplate Admin" Page options */
		...
		if ( ! function_exists( 'H5BP_site_js_setting' ) ):
			function H5BP_site_js_setting() {
				...
				echo '<code>' .H5BP_URL. '/js/scripts.js</code>';
				echo '<p>This allows you to maintain your own code that will not get overwritten during Theme updates.</p>';
				echo '<p>Selecting this option will add the following code to your pages just before the <code>&lt;/body&gt;</code>:</p>';
				echo '<code>&lt;script type=\'text/javascript\' src=\'' .H5BP_URL. '/js/scripts.js\'&gt;&lt;/script&gt;</code>';
				echo '<p>(The single quotes and no-longer-necessary attributes are from WP, would like to fix that... maybe next update...)</p>';
...
/*	4)	Create functions to add above elements to pages */
		...
		if ( ! function_exists( 'H5BP_add_favicon' ) ):
			function H5BP_add_favicon() {
				echo '<link rel="shortcut icon" href="'.home_url().'/favicon.ico">'.PHP_EOL;
			}
		...
		if ( ! function_exists( 'H5BP_add_ieshiv_script' ) ):
			function H5BP_add_ieshiv_script() {
				...
				echo '	<script src="//html5shiv.googlecode.com/svn/trunk/html5.js" async defer></script>'.PHP_EOL; // try getting from CDN
				echo '	<script>!window.html5 && document.write(unescape(\'%3Cscript async defer src="' .H5BP_URL. '/js/ieshiv.js'.$cache.'"%3E%3C/script%3E\'))</script>'.PHP_EOL; // fallback to local if CDN fails
				echo '<![endif]-->'.PHP_EOL;
			}
		...
		if ( ! function_exists( 'H5BP_add_jquery_script' ) ):
			function H5BP_add_jquery_script() {
				...
				#echo '<script src="' .H5BP_URL. '/js/jquery.js'.$cache.'" defer></script>'.PHP_EOL; // try getting from CDN
				echo '<script src="//ajax.googleapis.com/ajax/libs/jquery/'.$version.'/jquery.min.js" async defer></script>'.PHP_EOL; // try getting from CDN
		...
		if ( ! function_exists( 'H5BP_add_site_script' ) ):
			function H5BP_add_site_script() {
				$cache = H5BP_cache_buster();
				$debug = $_SERVER['HTTP_HOST'] == 'localhost' || isset($_GET['debug']) ? '.beauty' : '';				
				echo '<script src="' .H5BP_URL. '/js/scripts'.$debug.'.js'.$cache.'" async defer></script>'.PHP_EOL;
			}
...
function editorial_home() {
	$editorial = get_option($_GET['page'], array());
	if(!empty($_POST['post_category'])) {
		$content = fix_editorial($_POST['post_category']);
       	$editorial = $content;

        $just_saved = true;
        update_option($_GET['page'], $editorial);
    }

	include dirname(__FILE__) . '/editorial-destaque.php';
}

function fix_editorial($editorial) {
    $content = array();
	
	for ($i = 0; $i < count($editorial); $i++) {
		$content[] = array(
			'post_category' => $editorial['cat'.$i]
		);
	}
...
function editorial_menu() {
	add_menu_page('Destaques', 'Destaques', 'manage_options', 'destaques_home', 'editorial_home', '', '9');
}

add_action('admin_menu', 'editorial_menu');
