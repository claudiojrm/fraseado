...
if ( ! function_exists( 'boilerplate_filter_wp_title' ) ) :
	...
	function boilerplate_filter_wp_title( $title, $separator ) {
		...
		if ( is_search() ) {
			...
			$title = sprintf( __( 'Resultados de busca para %s', 'boilerplate' ), '"' . get_search_query() . '"' );
			// Add a page number if we're on page 2 or more:
			if ( $paged >= 2 )
				$title .= " $separator " . sprintf( __( 'Pagina %s', 'boilerplate' ), $paged );
		...
		if(is_home()) {
			...
			$title .= $separator . ' Fraseado';
		}

		// Add a page number if necessary:
		if ( $paged >= 2 || $page >= 2 )
			$title .= " $separator " . sprintf( __( 'Pagina %s', 'boilerplate' ), max( $paged, $page ) );
...
function search_form( $form ) {
	$form = '<form role="search" method="get" id="searchform" class="searchform clear" action="' . home_url( '/' ) . '" >
				<div>
					<input type="text" value="' . get_search_query() . '" name="s" id="s" placeholder="Frases e mensagens..." class="col-xs-10">
					<label for="searchsubmit" class="col-xs-2 submit">
